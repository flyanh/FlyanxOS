 ...
 * 
 * 在以后的代码中我们将会发现，信号处理牵涉到flyanx的所有部分。同时，这是
 * POSIX标准所要求的一个文件。所有POSIX需要的信号在flyanx中都定义了，但目
 * 前并不是所有的信号都得到了支持。例如，POSIX需要一些与作业控制，即把一
 * 个正在运行的程序放到后台和再放回来的能力，有关的信号，flyanx不支持作业
 * 控制，但是可能产生这些信号的程序可以移植到flyanx上。这些信号如果产生将
 * 被忽略。flyanx中还定义一些非POSIX信号和一些为了与旧的源码兼容的POSIX名
 * 的同义词。
 */
